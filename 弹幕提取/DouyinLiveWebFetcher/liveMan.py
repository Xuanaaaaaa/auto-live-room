...
class DouyinLiveWebFetcher:
    ...
    def __init__(self, live_id, abogus_file='a_bogus.js', enable_llm=False, dedup_ttl=60):
        """
        直播间弹幕抓取对象
        :param live_id: 直播间的直播id，打开直播间web首页的链接如：https://live.douyin.com/261378947940，
                        其中的261378947940即是live_id
        :param enable_llm: 是否启用 LLM 兜底解析（需要先在 弹幕分析脚本/danmaku_parser.py 的 LLM_CONFIG 填好 API key）
        :param dedup_ttl: 结构化结果去重窗口（秒）
        """
        ...
        self.danmu_file_path = os.path.join('danmu', f'{live_id}_{ts}.txt')

        os.makedirs('output', exist_ok=True)
        self.output_file_path = os.path.join('output', f'{live_id}_{ts}.jsonl')
        self._output_fp = open(self.output_file_path, 'a', encoding='utf-8', buffering=1)

        self.parser = DanmakuParser(enable_llm=enable_llm, dedup_ttl=dedup_ttl)
        self._parse_queue: queue.Queue = queue.Queue()
        self._worker_stop = object()
        self._parser_worker_thread = threading.Thread(
            target=self._parserWorker, daemon=True
        )
        self._parser_worker_thread.start()

        print(f"[init] 原始弹幕日志: {self.danmu_file_path}")
        print(f"[init] 结构化输出 (JSONL): {self.output_file_path}")
        print(f"[init] LLM 兜底: {'启用' if enable_llm else '关闭'}")
    ...
    def stop(self):
        ...
        if ws is not None:
            ws.close()
        self._parse_queue.put(self._worker_stop)
        self._parser_worker_thread.join(timeout=5)
        if self._output_fp and not self._output_fp.closed:
    ...
    def _parseChatMsg(self, payload):
        ...
        line = f"[{ts}] [{user_id}]{user_name}: {content}"
        print(f"【聊天msg】{line}")
        with open(self.danmu_file_path, 'a', encoding='utf-8') as f:
            f.write(line + '\n')
        self._parse_queue.put((str(user_id), user_name, content))

    @staticmethod
    def _build_query_payload(result) -> dict:
        keyword = result.keyword
        city = result.city
        return {
            "major": None,
            "education": result.education,
            "intention_job": [keyword] if keyword else None,
    ...
    def _parserWorker(self):
        """后台线程：消费队列 → 解析弹幕 → 命中则写 JSONL"""
        while True:
            item = self._parse_queue.get()
            if item is self._worker_stop:
                break
            user_id, user_name, content = item
            try:
            ...
            record = {
                ...
                **{k: v for k, v in asdict(result).items() if k != 'raw_text'},
                ...
                'query_payload': self._build_query_payload(result),
            }
            try:
                self._output_fp.write(json.dumps(record, ensure_ascii=False) + '\n')
            except Exception as e:
                print(f"[结构化输出写入异常] {e}")
